...
def explain_anomalies(X_anomalous, predict_fn, X_benign, background, out_file_path=None, model_to_optimize=None):
    ...
    :param background:      str background type, one of ['kmeans', 'zeros', 'mean', 'NN', 'optimized']
    ...
    elif background in ['zeros', 'mean', 'NN']:
    ...
    else:  # Assume background contains path to nearest neighbors, load individual background for all transactions
        raise ValueError("Variable background not one of ['kmeans', 'zeros', 'mean', 'NN', 'optimized']")
